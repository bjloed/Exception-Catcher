                                               ...
                                               uint64_t        hypercall_arg)
{
    if(hypercall_enabled) {
                if(cnt == 0)
                    write_virtual_memory(hypercall_arg, (uint8_t *)OVERWRITE_CALLBACK, 1, cpu);
                if(cnt == 1)
                    write_virtual_memory(hypercall_arg, (uint8_t *)RESTORE_CALLBACK, 1, cpu);
        // switch (get_current_mem_mode(cpu)) {
        //     case mm_64_l4_paging:
        //     case mm_64_l5_paging:
        //         if(cnt == 0)
    ...
    if (hypercall_enabled) {        
            ...
            GET_GLOBAL_STATE()->in_fuzzing_mode)
        {
            ...
            if (hypercall_arg & 0x8000000000000000ULL) {
                ...
                case 1:                    
                    //panic_count++;
                    //nyx_printf("kAFL panic_count plus 1\n");
                    set_crash_reason_auxiliary_buffer(
                        GET_GLOBAL_STATE()->auxilary_buffer,
                        (char *)"PANIC IN USER MODE!\n",
                        strlen("PANIC IN USER MODE!\n"));
                    //nyx_printf("KAFL panic routine success.\n");
                    break;
                default:
                    set_crash_reason_auxiliary_buffer(GET_GLOBAL_STATE()->auxilary_buffer,
                                                      (char *)"???\n",
                                            ...
                                            uint64_t        hypercall_arg)
{
    ...
    if(!strcmp("callstack", filename)) {
        nyx_printf("[CALLSTACK] panic_count: %d, worker_id: %d\n", panic_count, GET_GLOBAL_STATE()->worker_id);
        memset(filename, 0, 256);
        sprintf(filename, "callstack_%02d_%08d.txt", GET_GLOBAL_STATE()->worker_id, panic_count + 1);
    } else if(!strcmp("crash", filename)) {
        nyx_printf("[CRASH] panic_count: %d, worker_id: %d\n", panic_count, GET_GLOBAL_STATE()->worker_id);
        memset(filename, 0, 256);
        sprintf(filename, "crash_%02d_%08d.txt", GET_GLOBAL_STATE()->worker_id, panic_count + 1);
        panic_count += 1;
    }
    ...
    //                base_name) != -1);
    assert(asprintf(&host_path, "%s/dump/%s",
        GET_GLOBAL_STATE()->workdir_path, base_name) != -1);
...
err_out1:
    ...
    nyx_printf("[%02d-%03d] File Create Completed.\n",GET_GLOBAL_STATE()->worker_id, panic_count + 1 );
}
