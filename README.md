...
The **Exception Catcher** in kAFL serves the role of providing more detailed results for crashes that occur during userland software fuzzing. The Exception Catcher stores the crash call stack results caused by the payload, as well as the actual payload, under the dump folder.

## Usage

Move **UC.h** in the existing location of **nyx_api.h**, and apply the modified **hypercall.c** to the current code. After that, write the code by referring to **example_harness.c**.
